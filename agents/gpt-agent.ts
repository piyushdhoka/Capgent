...
async function main() {
  ...
  const model =
    process.env.OPENROUTER_GPT_MODEL ?? process.env.OPENROUTER_MODEL ?? "openai/gpt-4o";
  ...
  try {
    ...
    if (reg.identity_token) {
      await client.signGuestbook(
        reg.identity_token,
        `Signed by gpt-agent via ${model}. Challenge solved in ${solveMs}ms — XOR, reverse, slice, SHA-256, HMAC. Precisely executed.`,
        solveMs
      );
