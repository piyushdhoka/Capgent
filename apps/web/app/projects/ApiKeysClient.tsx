...
export function ApiKeysClient({
  ...
  function handleDelete(projectId: string, keyId: string) {
  ...
  const KeyRow = ({ k, showProject = true }: { k: ApiKeyWithProject; showProject?: boolean }) => (
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  ...
                  {k.label ? ` (${k.label})` : ""} will be permanently revoked. Any backend using it will lose access immediately.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                ...
                <AlertDialogAction
                  ...
                  onClick={() => handleDelete(k.project.id, k.id)}
                >
                  Delete key
                </AlertDialogAction>
              </AlertDialogFooter>
        ...
        <div className="flex items-center gap-3">
          ...
          <Tabs value={groupBy} onValueChange={(v) => setGroupBy(v as "key" | "project")}>
            <TabsList className="rounded-full bg-muted/40 border border-border/40">
              <TabsTrigger value="key" className="rounded-full px-4 text-xs">API key</TabsTrigger>
              <TabsTrigger value="project" className="rounded-full px-4 text-xs">Project</TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
      ...
      <div className="rounded-xl border border-border/60 bg-card overflow-hidden shadow-sm mt-2">
        ...
        <div className="divide-y divide-border/40">
          {filteredKeys.length === 0 ? (
            ...
            filteredKeys.map((k) => <KeyRow key={k.id} k={k} />)
          ) : (
            projects.map((p) => {
              ...
              return (
                <div key={p.id}>
                  ...
                  <div className="divide-y divide-border/40">
                    {pKeys.map((k) => <KeyRow key={k.id} k={k} showProject={false} />)}
                  </div>
                </div>
              )
            })
